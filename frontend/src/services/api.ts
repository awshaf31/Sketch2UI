 ...
 *
 * The bare `{ error: "string" }` form no longer exists anywhere in backend, so the
...
// exactly (backend/src/modules/admin/admin.routes.ts).
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  const res = await fetch(`${API_URL}${path}`, {
    ...init,
    // Required for the session cookie to be sent/stored: frontend and backend are
...
export const api = {
   ...
   * (QA audit DEF-008) — so it needs the project/page context, not just the storage key. */
