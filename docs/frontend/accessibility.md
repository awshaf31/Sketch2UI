...
| Surface | Behavior |
|---|---|
| Canvas — **highest-priority gap** | New keyboard mode: with a detection selected, arrow keys nudge it by 1px-equivalent (Shift+arrow for 10px-equivalent); `Tab`/`Shift+Tab` cycle selection through detections, then through that selection's four resize handles, in document order; `Enter` on a focused-but-unselected detection selects it; with a resize handle focused, arrow keys move that one corner by the same 1px/10px-with-Shift steps a mouse drag on it would (DEF-011, `docs/qa/MASTER_DEFECT_REGISTER.md`). This does not replace mouse drawing (still mouse/pointer-only, matching how a vector tool's draw tool works everywhere), but it closes the audit's flagged gap that *selection, correction, resize, and deletion* were entirely mouse-gated — those four now have full keyboard equivalents even though initial box *drawing* remains a pointer operation, matching the precedent every comparable annotation tool sets |
